                   ...
                   withCredentials([
                       ...
                       sh """
                           ...
                           for SCHEMA in "\${SCHEMAS[@]}"; do
                               ...
                               # Append schema to your dynamic environment URL
                               FULL_JDBC_URL="${env.snowflake_db_url}&schema=\$SCHEMA"
                               if [ ! -f "\$SNAPSHOT_FILE" ]; then
                                   echo "Creating baseline snapshot for \$SCHEMA..."
                                   liquibase --url="\$FULL_JDBC_URL" \\
                                     --username="\${DEV_USER}" \\
                                     --password="\${DEV_PASS}" \\
                                     snapshot --snapshotFormat=json --outputFile="\$SNAPSHOT_FILE"
                               ...
                               liquibase --url="\$FULL_JDBC_URL" \\
                                 ...
                                 --password="\${DEV_PASS}" \\
                                 diffChangeLog \\
                                 --referenceUrl="offline:snowflake?snapshot=\$SNAPSHOT_FILE" \\
                                 --changeLogFile="\$DIFF_FILE" \\
                                 --format=sql \\
                                 --diffTypes="tables,views,columns,indexes,sequences"
                               if [ -s "\$DIFF_FILE" ] && grep -qi "CREATE\\|ALTER\\|DROP" "\$DIFF_FILE"; then
                                   echo "Incremental changes found in \$SCHEMA!"
                                   # Update the offline snapshot to the new state
                                   liquibase --url="\$FULL_JDBC_URL" \\
                                     --username="\${DEV_USER}" \\
                                     --password="\${DEV_PASS}" \\
                                     snapshot --snapshotFormat=json --outputFile="\$SNAPSHOT_FILE"
                           ...
                           if [ "\$CHANGES_DETECTED" = true ]; then
                               ...
                               # Push using the exact variable replacement logic from your checkout stage
